    ...
    def __call__(self, img):
        ...
        if img_group[0].mode == 'L':
            ...
            if self.roll:
                ...
                # print(np.stack([np.array(x) for x in img_group], axis=0).shape)

                # Stack the images in the group along the first axis: (T, H, W, C)
                return np.stack([np.array(x) for x in img_group], axis=0), label
